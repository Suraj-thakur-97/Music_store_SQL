...
--Q4: Which city has the best coustmers? we would like to throw a promotional Music Festival in the 
--city we made the most money. Write a query that returns one city that has the highest sum of 
--invoices totals. return both the city name & sum of all invoice totals.
...
--Q5 Who is the best coustmer? the customer who has spent the most money will be declared the 
...
--returns the artist name and total track count of the top rock bands.
...
--Q9 find how much amount spent by each customer on artist ? write a query to return customer 
--nmae, artist name and total spent. 
...
--spent. for countries where with the top amount spent is sheared provide all customers who spent 
--this amount.
...
JOIN 
    country_max_spending cms ON cc.billing_country = cms.billing_country
WHERE 
...
ORDER BY 1,5 DESC),
country_max_spending AS(
SELECT billing_country, MAX(toatl_spending)AS max_spending
FROM customer_with_country
GROUP BY billing_country)
SELECT cc.billing_country,cc.total_spending, cc.first_name., cc.last_name, cc.customer_id
FROM customer_with_country cc
JOIN country_max_spending as
ON cc.billing_country = ns.billing_country
WHERE cc.total_spending = ns.max_spending
ORDER BY 1;